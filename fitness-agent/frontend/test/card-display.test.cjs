...
test("chat cards and message bubbles keep long generated text inside their containers", () => {
  ...
  assertLongTextSafe(".agent-card-status-grid strong", ruleFor(".agent-card-status-grid strong"));
  assertLongTextSafe(".revision-compare-grid p", ruleFor(".revision-compare-grid p"));
});

test("agent card layouts use stable responsive grids", () => {
  assert.match(
    ruleFor(".agent-card-status-grid"),
    /grid-template-columns:\s*repeat\(auto-fit,\s*minmax\(118px,\s*1fr\)\);/,
    "Agent status grid should not resize around dynamic text."
  );
  assert.match(
